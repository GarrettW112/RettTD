import { Wisp, Skeleton, Poltergheist, Golem, Zombie, NecNovice, ZombieCaptain } from './Enemy.js';
...
export class Environment {
    ...
    update(enemies) {
        ...
        if (this.level == 3) {
            if (this.envtime % 60 == 0 && this.envtime < 1202) {
                let rand = Math.floor(Math.random() * 4);
                if (this.envtime % 300 == 0) {
                    if (rand == 0) {
                    enemies.push(new Poltergheist(Math.floor(Math.random() * this.width), 0));
                    }
                    if (rand == 1) {
                        enemies.push(new Poltergheist(Math.floor(Math.random() * this.width), this.height));
                    }
                    if (rand == 2) {
                        enemies.push(new Poltergheist(0, Math.floor(Math.random() * this.height)));
                    }
                    if (rand == 3) {
                        enemies.push(new Poltergheist(this.width, Math.floor(Math.random() * this.height)));
                    }
                }
                else if (this.envtime % 240 == 0) {
                    if (rand == 0) {
                    enemies.push(new Skeleton(Math.floor(Math.random() * this.width), 0));
                    }
                    if (rand == 1) {
